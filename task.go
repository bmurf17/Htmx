...
func fetchTask(ID int) (Item, error) {
	var item Item
	err := DB.QueryRow("select id, title, completed from tasks order by position where id = (?)", ID).Scan(&item.ID, &item.Title, &item.Completed)
	if err != nil {
		return Item{}, err
	}
	return item, nil
...
func updateTask(ID int, title string) (Item, error) {
	var item Item
	err := DB.QueryRow("update tasks set title = (?) where id = (?)", title, ID).Scan(&item.ID, &item.Title, &item.Completed)
	if err != nil {
		return Item{}, err
	}
	return item, nil
...
func deleteTasks(ctx context.Context, ID int) error {
	...
	for idx, id := range ids {
		_, err := DB.Exec("update tasks set position = (?) where id = (id)", idx, id)
		if err != nil {
			return err
		}
	}
